...
class PlotRenderer:
    ...
    def __init__(self, parent_widget, theme):
        ...
        #self.islat = self.plot_manager.islat
        ...
        self.fig1 = plt.Figure(figsize=(5, 3.5))
        self.ax1 = self.fig1.add_subplot(111)
        self.canvas1 = FigureCanvasTkAgg(self.fig1, master=self.subplot_frames['main'])
        ...
        self.fig2 = plt.Figure(figsize=(5, 3.5))
        self.ax2 = self.fig2.add_subplot(111)
        self.canvas2 = FigureCanvasTkAgg(self.fig2, master=self.subplot_frames['line'])
        ...
        self.fig3 = plt.Figure(figsize=(10, 3.5))
        self.ax3 = self.fig3.add_subplot(111)
        self.canvas3 = FigureCanvasTkAgg(self.fig3, master=self.subplot_frames['population'])
        self.canvas3.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        
        # Store all canvases for easy access
        self.canvases = [self.canvas1, self.canvas2, self.canvas3]
        self.canvas = self.canvas1  # Keep backward compatibility
    ...
    def update_all_plots(self, data):
        ...
        if 'wave_data' in data and 'flux_data' in data:
            self.render_main_spectrum_plot(
                data['wave_data'], 
                data['flux_data'], 
                data.get('molecules', []), 
        ...
        if 'line_wave' in data and 'line_flux' in data:
            self.render_line_inspection_plot(data['line_wave'], data['line_flux'], line_label=data.get('line_label'))
        
        # Render population diagram if active molecule is set
        if 'active_molecule' in data:
            self.render_population_diagram(data['active_molecule'], wave_range=data.get('wave_range'))
