...
class FileHandler:
    """
    FileHandler class provides methods to handle file operations such as reading and writing user settings,
    reading from CSV files, and saving data.
    """
    ...
    def get_main_directory():
        """Get the main directory for the project."""
        return os.path.dirname(os.path.abspath(__file__))
    
    @staticmethod
    def get_data_directory():
        """Get the directory where data files are stored."""
        return os.path.join(FileHandler.get_main_directory(), "DATAFILES")

    @staticmethod
    def get_spectral_file_directory():
        """Get the directory where spectral files are stored."""
        return os.path.join(FileHandler.get_data_directory(), "SPECTRAL_DATA")
    
    @staticmethod
    def get_example_data_directory():
        """Get the directory where example data files are stored."""
        return os.path.join(FileHandler.get_data_directory(), "EXAMPLE-data")
    
    @staticmethod
    def get_hitran_data_directory():
        """Get the directory where HITRAN data files are stored."""
        return os.path.join(FileHandler.get_data_directory(), "HITRANdata")
    
    def read_user_settings(self, file_path=user_configuration_file_path, file_name=user_configuration_file_name, theme_file_path=theme_file_path):
        """Read user settings from a JSON file and return as dictionary."""
        
        relative_file = os.path.join(file_path, file_name)
        file = os.path.abspath(relative_file)
        
        if os.path.exists(file):
            user_settings = pd.read_json(file, typ='series').to_dict()
        
        # Append theme information to the user settings dictionary
        theme_file = os.path.join(theme_file_path, f"{user_settings['theme']}.json")
        if os.path.exists(theme_file):
            theme_settings = pd.read_json(theme_file, typ='series').to_dict()
            user_settings["theme"] = theme_settings
        
        return user_settings
    
    def check_Hitran(self, file_path=None):
        """Check if the HITRAN data directory exists."""
        if file_path is None:
            file_path = self.get_hitran_data_directory()
        return os.path.exists(file_path)
    
    def read_spectrum_file(self, file_path=None):
        """Read a spectrum file and return its contents."""
        if file_path is None:
            file_path = self.get_spectral_file_directory()
